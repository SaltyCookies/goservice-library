...
func (r *TodoItemPostgres) Create(listId int, item goservice.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId int, listId int) ([]goservice.TodoItem, error) {
	var items []goservice.TodoItem
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (goservice.TodoItem, error) {
	var item goservice.TodoItem
...
func (r *TodoItemPostgres) Update(userId int, itemId int, input goservice.UpdateItemInput) error {
	...
	query := fmt.Sprintf("UPDATE %s ti SET %s FROM %s li, %s ul WHERE ti.id = li.item_id AND li.list_id = ul.list_id AND ti.id = $%d AND ul.user_id = $%d", todoItemsTable, setQuery, listsItemsTable, usersListsTable, argId, argId+1)
	args = append(args, itemId, userId)

	_, err := r.db.Exec(query, args[0], args[1], args[2], args[3], args[4])
	return err
}
